...
async function fetchInterpretation(id:string) {
    try {
     const interpretation = await database.getDocument(process.env.NEXT_PUBLIC_APPWRITE_DATABASE_ID as string, "6694e74d003dfb5fa0cb", id)   
...
//Delete
 ...
 async function deleteInterpretation(id:string) {
    try {
        const response = await database.deleteDocument(process.env.NEXT_PUBLIC_APPWRITE_DATABASE_ID as string, "6694e74d003dfb5fa0cb", id)
        return response;
    } catch (error) {
 ...
 export async function GET(req:Request, {params}: {params:{id:string}}){
    try {
        ...
        return NextResponse.json({error: "Error to fetch interpretation"},{status:500})      
    }
 }
 ...
 export async function DELETE(req:Request, {params}: {params:{id:string}}){
    try {
        ...
        return NextResponse.json({error: "Error to delete interpretation"},{status:500})      
    }
 }
